...
public class LevelGenerator : EditorWindow 
{
	...
	void generateParent()
	{
		...
		for(int i=0; i<objectList.Count; i++) // for each object created
		{
			if(!GameObject.Find(objectList[i]))// if the folder doesn't exist yet
                Parento = new GameObject(objectList[i]);// create the folder
            Parento.transform.parent = level.transform;// set the object in existent folder
		}
	...
	void Generate()
	{
		...
		for(int ty=0; ty<colorMap.GetLength(1) ; ty++) // read the colors in the color map in the Y axix //0,0
			{			
				for(int tx=0; tx<colorMap.GetLength(0); tx++) // read the colors in the color map in the X axix
				{
                //would be good if all objects were 'similar' sized
					instPos = new Vector3((tx), ty, (0)); // set the object's position based in the color position in the table
